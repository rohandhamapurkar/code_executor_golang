...
func UpdateSnippetHandler(ctx *gin.Context) {

	body := &structs.UpdateSnippetReqBody{}
	...
	err := snippetsService.UpdateSnippet(body, userClaims.Sub)
	if err != nil {
		ctx.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": constants.ERROR_SNIPPET_UPDATION, "message": err.Error()})
		return
	}

	ctx.JSON(http.StatusOK, gin.H{"message": constants.SNIPPET_UPDATED})
